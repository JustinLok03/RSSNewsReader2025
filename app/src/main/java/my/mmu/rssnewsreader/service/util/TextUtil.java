...
public class TextUtil {
    ...
    public void handleLanguageMismatch(String content, String feedLanguage, BiConsumer<String, String> onMismatchDetected) {
        identifyLanguageRx(content)
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.io())
                .subscribe(identifiedLanguage -> {
                    Log.d(TAG, "Feed Language: " + feedLanguage);
                    Log.d(TAG, "Identified Language: " + identifiedLanguage);

                    if (!feedLanguage.equals(identifiedLanguage)) {
                        // Notify about the mismatch with both identified and feed languages
                        onMismatchDetected.accept(identifiedLanguage, feedLanguage);
                    } else {
                        // No mismatch; pass the feed language
                        onMismatchDetected.accept(feedLanguage, feedLanguage);
                    }
                }, throwable -> {
                    Log.e(TAG, "Error during language mismatch handling", throwable);
                    // On error, fallback to feed language
                    onMismatchDetected.accept(feedLanguage, feedLanguage);
                });
    }
